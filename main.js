...
ipcMain.on('fs', (event, obj) => {
    ...
    if(obj.situacao == "create"){
        var createStream = fs.createWriteStream(path.join(__dirname + "/tarefas/" + uuid.v4() + ".txt")) //Cria Arquivo
        ...
        if(!obj.filter){
            var qtdTasks = 0
            fs.readdirSync("./tarefas").forEach(arquivo => {
                //Abre e Lê Arquivo na Codificação UTF-8
                var dados = fs.readFileSync(path.join(__dirname + "/tarefas/" + arquivo), {encoding:'utf8', flag:'r'})
        ...
        var dados = fs.readFileSync(path.join(__dirname + "/tarefas/" + obj.file_id + ".txt"), {encoding:'utf8', flag:'r'})
        if(dados == ""){
            respondeSemTarefa()
        }    
        else {
        ...
        fs.writeFile(path.join(__dirname + "/tarefas/" + obj.arquivo + ".txt"), conteudo, (erro) => {
        ...
        fs.writeFile(path.join(__dirname + "/tarefas/" + obj.file_id + ".txt"), conteudo, (erro) => {
                enviaMsg("finish", "Tarefa Concluída Com Sucesso!") 
        })
    }
    else if(obj.situacao == "delete"){
        fs.unlink(path.join(__dirname + "/tarefas/" + obj.file + ".txt"), (erro) => {
